...
userRouter.get("/users", async (req: Request, res: Response) => {
    try {
        ...
        return res.status(StatusCodes.OK).json({totalUsers: allUsers.length, allUsers});
...
userRouter.post("/register", async (req: Request, res: Response) => {
    try {
        ...
        if (user){
            return res.status(StatusCodes.CONFLICT).json({error: "This email has already benn registered.."});
        }
...
userRouter.post("/login", async (req: Request, res: Response) => {
    try{
        ...
        const comparePassword = await bcrypt.compare(email, password);
...
userRouter.put("/users/:id", async (req: Request, res: Response) => {
    try {
        ...
        if(!getUser){
            return res.status(StatusCodes.NOT_FOUND).json({error: `No user with the id of ${req.params.id}`});
        }
